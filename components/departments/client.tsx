...
export default function DepartmentsClient({ departments }: DepartmentsClientProps) {
    ...
    const handleFormSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            const res = await fetch(
                modalMode === "insert"
                    ? "/api/departments"
                    : `/api/departments/${editingData?.id}`,
                {
    ...
    const handleRemovePosition = async () => {
        ...
        try {
            const res = await fetch(`/api/departments/${removeData?.id}`, {
                method: "DELETE"
            });
            const data = await res.json();
            if (res.ok) {
    ...
    const filteredDepartments = departments.filter((d) => {
        ...
        return d.title.toLowerCase().includes(normalizedSearch);
    });
                            ...
                            <ol className="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                                ...
                                <li>
                                    <div className="flex items-center">
                                        ...
                                        <span className="ml-1 text-gray-400 md:ml-2 dark:text-gray-500">Departments</span>
                                    </div>
                                </li>
                            </ol>
                        </nav>
                        <h1 className="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Departments</h1>
                    </div>
                    <div className="sm:flex">
                        ...
                        <div className="flex items-center ml-auto space-x-2 sm:space-x-3">
                            <button type="button" onClick={() => { setModalOpen(true); setModalMode("insert"); setEditingData(null); }} className="inline-flex items-center justify-center w-1/2 px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-green-400 hover:bg-green-500 focus:ring-4 focus:ring-green-300 sm:w-auto dark:bg-green-700 dark:bg-green-400 dark:hover:bg-green-500 dark:focus:ring-primary-800">
                                <PlusCircle className="w-5 h-5 mr-2 -ml-1"/> Add department
                            </button>
                        </div>
                    </div>
                                ...
                                <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                                    {paginatedDepartments.map((department) => (
                                        <tr key={department.id} className="hover:bg-gray-100 dark:hover:bg-gray-700">
                                            <td className="p-2 text-sm text-gray-900 whitespace-nowrap dark:text-white">{department.name}</td>
                                            <td className="p-2 space-x-2 whitespace-nowrap">
                                                <button type="button" onClick={() => { setModalOpen(true); setModalMode("update"); setEditingData(department); }} className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                                                    <SquarePen className="w-4 h-4"/>
                                                </button>
                                                <button type="button" onClick={() => { setRemoveData(department); setRemoveModalOpen(true); }} className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-800 focus:ring-4 focus:ring-red-300 dark:focus:ring-red-900">
                            ...
                            <div className="relative bg-white rounded-lg shadow dark:bg-gray-800">
                                <div className="flex items-start justify-between p-5 border-b rounded-t dark:border-gray-700">
                                    <h3 className="text-xl font-semibold dark:text-white">
                                        {modalMode === "insert" ? "Add new department" : "Edit department"}
                                    </h3>
                                    ...
                                    <div className="p-6 space-y-6">
                                        ...
                                        <div className="grid grid-cols-4 gap-6">
                                            <div className="col-span-6">
                                                ...
                                                <input type="text" id="name" name="name" defaultValue={editingData?.name ?? ""} className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Name" autoComplete="off" required/>
                                            </div>
                                        </div>
                                    </div>
                                    {modalMode === "update"
                                ...
                                <div className="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                    <div className="sm:flex sm:items-start gap-4">
                                        ...
                                        <div className="mt-3 sm:mt-0 sm:ml-4 sm:text-left">
                                            <h3 className="text-base font-semibold text-gray-900 dark:text-gray-200">Remove department</h3>
                                            <div className="mt-2">
                                                <p className="text-sm text-start text-gray-500 dark:text-gray-400">Are you sure you want to remove this department? This action cannot be undone.</p>
                                            </div>
                                        </div>
                                    </div>
