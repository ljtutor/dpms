...
export default function PositionsClient({ positions }: PositionsClientProps) {
    ...
    const handleRemovePosition = async () => {
        ...
        try {
            const res = await fetch(`/api/positions/${removeData?.id}`, {
                method: "DELETE"
            });
            const data = await res.json();
            if (res.ok) {
            ...
            <div className="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5 dark:bg-gray-800 dark:border-gray-700">
                <div className="w-full mb-1">
                    <div className="mb-4">
                        ...
                        <h1 className="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Positions</h1>
                    </div>
                                    ...
                                    {paginatedPositions.map((position) => (
                                        <tr key={position.id} className="hover:bg-gray-100 dark:hover:bg-gray-700">
                                            ...
                                            <td className="p-2 space-x-2 whitespace-nowrap">
                                                <button type="button" onClick={() => { setModalOpen(true); setModalMode("update"); setEditingData(position); }} className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                                                    <SquarePen className="w-4 h-4"/>
                                                </button>
                                                <button type="button" onClick={() => { setRemoveData(position); setRemoveModalOpen(true); }} className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-800 focus:ring-4 focus:ring-red-300 dark:focus:ring-red-900">
                            ...
                            <div className="relative bg-white rounded-lg shadow dark:bg-gray-800">
                                <div className="flex items-start justify-between p-5 border-b rounded-t dark:border-gray-700">
                                    <h3 className="text-xl font-semibold dark:text-white">
                                        {modalMode === "insert" ? "Add new position" : "Edit position"}
                                    </h3>
                                    ...
                                    <div className="p-6 space-y-6">
                                        ...
                                        <div className="grid grid-cols-4 gap-6">
                                            <div className="col-span-2">
                                                ...
                                                <input type="text" id="title" name="title" defaultValue={editingData?.title ?? ""} className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Title" autoComplete="off" required/>
                                            </div>
                                        </div>
                                    </div>
                                    {modalMode === "update"
