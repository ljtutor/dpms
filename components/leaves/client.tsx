...
export default function LeavesClient({ leaves }: LeavesClientProps) {
    ...
    const handleFormSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        const accrualRateManager = formData.get("accrualRateManager") as string;
        const accrualRateUser = formData.get("accrualRateUser") as string;
        const accrualPeriod = formData.get("accrualPeriod") as string;

        try {
            const res = await fetch(
                modalMode === "insert"
                    ? "/api/leaves"
                    : `/api/leaves/${editingData?.id}`,
                {
    ...
    const handleRemoveShift = async () => {
        ...
        try {
            const res = await fetch(`/api/leaves/${removeData?.id}`, {
                method: "DELETE"
            });
            const data = await res.json();
            if (res.ok) {
    ...
    const filteredLeaves = leaves.filter((l) => {
        ...
        const hay = [l.type ?? ""].join(" ").toLowerCase();
                            ...
                            <ol className="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                                ...
                                <li>
                                    <div className="flex items-center">
                                        ...
                                        <span className="ml-1 text-gray-400 md:ml-2 dark:text-gray-500">Leaves</span>
                                    </div>
                                </li>
                            </ol>
                        </nav>
                        <h1 className="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Leaves</h1>
                    </div>
                    <div className="sm:flex">
                        ...
                        <div className="flex items-center ml-auto space-x-2 sm:space-x-3">
                            <button type="button" onClick={() => { setModalOpen(true); setModalMode("insert"); setEditingData(null); }} className="inline-flex items-center justify-center w-1/2 px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-green-400 hover:bg-green-500 focus:ring-4 focus:ring-green-300 sm:w-auto dark:bg-green-700 dark:bg-green-400 dark:hover:bg-green-500 dark:focus:ring-primary-800">
                                <PlusCircle className="w-5 h-5 mr-2 -ml-1"/> Add leave
                            </button>
                        </div>
                    </div>
                            ...
                            <table className="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                                <thead className="bg-gray-100 dark:bg-gray-700">
                                    <tr>
                                        ...
                                        <th scope="col" className="p-4 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400">Accrual Rate (Manager)</th>
                                        <th scope="col" className="p-4 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400">Accrual Rate (User)</th>
                                        <th scope="col" className="p-4 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400">Accrual Period</th>
                                ...
                                <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                                    {paginatedLeaves.map((leave) => (
                                        <tr key={leave.id} className="hover:bg-gray-100 dark:hover:bg-gray-700">
                                            <td className="p-2 text-sm text-gray-900 whitespace-nowrap dark:text-white">{leave.type}</td>
                                            <td className="p-2 text-sm text-gray-900 whitespace-nowrap dark:text-white">{leave.accrualRateManager}</td>
                                            <td className="p-2 text-sm text-gray-900 whitespace-nowrap dark:text-white">{leave.accrualRateUser}</td>
                                            <td className="p-2 text-sm text-gray-900 whitespace-nowrap dark:text-white">{leave.accrualPeriod ? leave.accrualPeriod.charAt(0) + leave.accrualPeriod.slice(1).toLowerCase() : ""}</td>
                                            <td className="p-2 space-x-2 whitespace-nowrap">
                                                ...
                                                <button type="button" onClick={() => { setRemoveData(leave); setRemoveModalOpen(true); }} className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-800 focus:ring-4 focus:ring-red-300 dark:focus:ring-red-900">
                            ...
                            <div className="relative bg-white rounded-lg shadow dark:bg-gray-800">
                                <div className="flex items-start justify-between p-5 border-b rounded-t dark:border-gray-700">
                                    <h3 className="text-xl font-semibold dark:text-white">
                                        {modalMode === "insert" ? "Add new leave" : "Edit leave"}
                                    </h3>
                                ...
                                <form onSubmit={handleFormSubmit}>
                                    <div className="p-6 space-y-6">
                                        ...
                                        <div className="grid grid-cols-4 gap-6">
                                            <div className="col-span-2">
                                                <label htmlFor="type" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Type <span className="text-red-500">*</span></label>
                                                <input type="text" id="type" name="type" defaultValue={editingData?.type ?? ""} className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Type" autoComplete="off" required/>
                                            </div>
                                        </div>
                                        <div className="grid grid-cols-4 gap-6">
                                            <div className="col-span-2">
                                                <label htmlFor="accrualRateManager" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Accrual Rate (Manager)</label>
                                                <input type="number" id="accrualRateManager" name="accrualRateManager" step="0.01" defaultValue={editingData?.accrualRateManager ?? ""} className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"/>
                                            </div>
                                            <div className="col-span-2">
                                                <label htmlFor="accrualRateUser" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Accrual Rate (User)</label>
                                                <input type="number" id="accrualRateUser" name="accrualRateUser" step="0.01" defaultValue={editingData?.accrualRateUser ?? ""} className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"/>
                                            </div>
                                        </div>
                                        <div className="grid grid-cols-4 gap-6">
                                            <div className="col-span-2">
                                            <label htmlFor="accrualPeriod" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Accrual Period</label>
                                                <select id="accrualPeriod" name="accrualPeriod" defaultValue={editingData?.accrualPeriod ?? ""} className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                    ...
                                    <div className="items-center p-6 flex justify-end gap-4 border-t border-gray-200 rounded-b dark:border-gray-700">
                                        ...
                                        <button type="button" onClick={() => setModalOpen(false)} className="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 border border-gray-300 dark:bg-gray-700 dark:text-white dark:border-gray-600 sm:mt-0 sm:w-auto">Cancel</button>
                                    </div>
                                </form>
                            </div>
                        </div>
