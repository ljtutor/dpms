...
export default function UsersClient({ users, positions }: UsersClientProps) {
    ...
    const [removeData, setRemoveData] = useState<any | null>(null);
    ...
    const handleRemoveUser = async () => {
        if (!removeData?.id) return;
        setSuccess("");
        setError("");

        try {
            const res = await fetch(`/api/users/${removeData?.id}`, {
                method: "DELETE"
            });
            const data = await res.json();
            if (res.ok) {
    ...
    const filteredUsers = users.filter((u) => {
        ...
        const hay = [
            u.employeeInformation?.firstName ?? "",
            u.employeeInformation?.middleName ?? "",
            u.employeeInformation?.lastName ?? "",
            u.email,
            u.companyInformation?.position?.title ?? "",
            u.role,
            ...
            <div className="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5 dark:bg-gray-800 dark:border-gray-700">
                <div className="w-full mb-1">
                    <div className="mb-4">
                        ...
                        <h1 className="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Users</h1>
                    </div>
                    <div className="sm:flex">
                        ...
                        <div className="flex items-center ml-auto space-x-2 sm:space-x-3">
                            <button type="button" onClick={() => { setModalOpen(true); setModalMode("insert"); setEditingData(null); }} className="inline-flex items-center justify-center w-1/2 px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-green-400 hover:bg-green-500 focus:ring-4 focus:ring-green-300 sm:w-auto dark:bg-green-700 dark:bg-green-400 dark:hover:bg-green-500 dark:focus:ring-primary-800">
                                ...
                                <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                                    {paginatedUsers.map((user) => (
                                        <tr key={user.id} className="hover:bg-gray-100 dark:hover:bg-gray-700">
                                            <td className="p-2 text-sm text-gray-900 whitespace-nowrap dark:text-white">{user.employeeInformation?.firstName ?? ""}</td>
                                            <td className="p-2 text-sm text-gray-900 whitespace-nowrap dark:text-white">{user.employeeInformation?.middleName ?? ""}</td>
                                            <td className="p-2 text-sm text-gray-900 whitespace-nowrap dark:text-white">{user.employeeInformation?.lastName ?? ""}</td>
                                            ...
                                            <td className="p-2 space-x-2 whitespace-nowrap">
                                                <button type="button" onClick={() => { setModalOpen(true); setModalMode("update"); setEditingData(user); }} className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                                                    <UserPen className="w-4 h-4"/>
                                                </button>
                                                <button type="button" onClick={() => { setRemoveData(user); setRemoveModalOpen(true); }} className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-800 focus:ring-4 focus:ring-red-300 dark:focus:ring-red-900">
                            ...
                            <div className="relative bg-white rounded-lg shadow dark:bg-gray-800">
                                <div className="flex items-start justify-between p-5 border-b rounded-t dark:border-gray-700">
                                    <h3 className="text-xl font-semibold dark:text-white">
                                        {modalMode === "insert" ? "Add new user" : "Edit user"}
                                    </h3>
                                    <button type="button" onClick={() => setModalOpen(false)} className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-700 dark:hover:text-white">
                                        <X className="w-5 h-5"/>
                                    </button>
                                </div>
                                <form onSubmit={handleFormSubmit}>
                                    <div className="p-6 space-y-6">
                                        ...
                                        <div className="grid grid-cols-3 gap-6">
                                            ...
                                            <div className="col-span-1">
                                                ...
                                                <input type="text" id="lastName" name="lastName" defaultValue={editingData?.employeeInformation?.lastName ?? ""} className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Last Name" autoComplete="off" required/>
                                            </div>
                                        </div>
                                        <div className="grid grid-cols-3 gap-6">
                                            <div className="col-span-2">
                                                ...
                                                <input type="email" id="email" name="email" defaultValue={editingData?.email ?? ""} className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="name@dataplus.com.ph" autoComplete="off" required/>
                                            </div>
                                            <div className="col-span-1">
                                                <label htmlFor="birthday" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Birthday</label>
                                                <input type="date" id="birthday" name="birthday" defaultValue={editingData?.employeeInformation?.birthday ? editingData.employeeInformation.birthday.toISOString().split("T")[0] : ""} className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Birthday" autoComplete="off"/>
                                            </div>
                                        </div>
                                        <div className="grid grid-cols-3 gap-6">
                                            <div className="col-span-1">
                                                <label htmlFor="positionId" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Position</label>
                                                <select id="positionId" name="positionId" defaultValue={editingData?.companyInformation?.positionId ?? ""} className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                        ...
                                        <div className="grid grid-cols-3 gap-6">
                                            <div className="col-span-1">
                                                ...
                                                <select id="role" name="role" defaultValue={editingData?.role ?? Role.USER} className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
                                            ...
                                            {modalMode === "update"
                                                ? <div className="col-span-1">
                                                    ...
                                                    <select id="isActive" name="isActive" defaultValue={editingData?.isActive ? "true" : "false"} className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
                                        ...
                                        ? <div className="p-6 space-y-6 border-t border-gray-200 rounded-b dark:border-gray-700">
                                            <div className="grid grid-cols-3 gap-6">
                                                <div className="col-span-1">
                                                    ...
                                                    <input type="text" id="createdAt" defaultValue={editingData?.createdAt ? editingData?.createdAt.toISOString().slice(0, 19).replace("T", " ") : ""} className="shadow-sm bg-gray-200 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-900 dark:border-gray-700 dark:placeholder-gray-400 dark:text-gray-400" disabled/>
                                                </div>
                                                <div className="col-span-1">
                                                    <label htmlFor="updatedAt" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Updated at</label>
                                                    <input type="text" id="updatedAt" defaultValue={editingData?.updatedAt ? editingData?.updatedAt.toISOString().slice(0, 19).replace("T", " ") : ""} className="shadow-sm bg-gray-200 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-900 dark:border-gray-700 dark:placeholder-gray-400 dark:text-gray-400" disabled/>
                                                </div>
                                            </div>
                                        </div>
                                        : ""
                                    ...
                                    <div className="items-center p-6 flex justify-end gap-4 border-t border-gray-200 rounded-b dark:border-gray-700">
                                        ...
                                        <button type="button" onClick={() => setModalOpen(false)} className="inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 border border-gray-300 dark:bg-gray-700 dark:text-white dark:border-gray-600 sm:mt-0 sm:w-auto">Cancel</button>
                                    </div>
                                </form>
                            </div>
                        </div>
